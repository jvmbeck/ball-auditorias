...
import { fetchFailuresOverTime } from 'src/services/audit';
...
const chartState = ref<FailuresOverTimeData>({
  labels: [],
  data: [],
  countsByDate: {},
});
...
onMounted(async () => {
  ...
  try {
    chartState.value = await fetchFailuresOverTime();
  } catch (err: unknown) {
    error.value = err instanceof Error ? err.message : 'Unable to load failure trend data.';
  } finally {
    loading.value = false;
  }
});
