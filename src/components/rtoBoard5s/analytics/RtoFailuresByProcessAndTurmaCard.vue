...
const loading = computed(() => analyticsStore.checklistByProcessAndTurmaLoading);
const error = computed(() => analyticsStore.checklistByProcessAndTurmaError);
const chartState = computed(() => analyticsStore.checklistFailuresByProcessAndTurma);
...
watch(
  ...
  async (days) => {
    try {
      await analyticsStore.loadFailuresByProcessAndTurmaByType('rto', false, days);
    } catch {
